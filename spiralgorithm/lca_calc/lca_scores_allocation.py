...
def dict_LCA_score_allocation (recipe):
    ...
    if all(np.array(list(recipe['analyses']['allocation'].values())) == False):
        print('\nWARNING: To perform an allocation on for S2, S12, or S123, set any of the "allocation" items to "True" in the recipe (input > analyses > allocation).')
