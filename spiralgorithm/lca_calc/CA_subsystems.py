...
def relative_stacked_bar_plot_CA_subsystem (recipe, master_output):
    ...
    print('Figure saved at: %s' %str(recipe['rdir'] + "/CA_subsystem/CA-subsystem-relative-stacked-bar-chart-period%s.pdf"%period_number))
...
def bar_chart_CA_subsystems_specific_IC (recipe, master_output):
    ...
    for method in recipe['CA_subsystems']['bar_chart_one_ic']['lcia_methods']: 
        ...
        ax.set_xticks(x_pos)
        ax.set_xticklabels([i.replace('_',' ') for i in subsystem_lst], rotation=45)
